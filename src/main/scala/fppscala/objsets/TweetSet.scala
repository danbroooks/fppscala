...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val rec = (left union right)
    if (rec.isEmpty) elem
    else rec.mostRetweeted match {
      case r => if (r.retweets > elem.retweets) r else elem
    }
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => google.exists(tweet.text.contains(_)))

  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => apple.exists(tweet.text.contains(_)))
